...
void update() {
    ...
    if (player1.y + PLAYER_HEIGHT + player1.yVelocity <= SCREEN_HEIGHT - WALL_HEIGHT) {
    ...
    if (state.joy_x_axis > 140 && player1.x + PLAYER_ACTUAL_WIDTH < SCREEN_WIDTH - WALL_WIDTH)
        player1.x += 2;

    // Check for movement to left (only move when not against the wall)
    else if (state.joy_x_axis < 100 && player1.x > WALL_WIDTH)
        player1.x -= 2;
...
void updateCollision() {
    for (auto &wall: walls) {    
        ...
        if (player1.y <= wall.y + wall.height && player1.x + PLAYER_ACTUAL_WIDTH - player1.yVelocity >= wall.x &&
            player1.x <= wall.x + wall.width) {
                player1.y = wall.y + wall.height;
                player1.yVelocity = GRAVITY;
        }

        //Checking if player is not underneath the wall and above the wall.
        if (wall.y + wall.height > player1.y && player1.y + PLAYER_HEIGHT >= wall.y) 
        {
            //Right side player collision detection.
            if (player1.x + PLAYER_ACTUAL_WIDTH >= wall.x && player1.x <= wall.x + wall.width / 2) {
                player1.x = wall.x - PLAYER_ACTUAL_WIDTH;
            }

            //Left side player collision detection.
            if (player1.x <= wall.x + wall.width && player1.x >= wall.x + wall.width / 2) {
                player1.x = wall.x + wall.width;
            }
        }
    }
